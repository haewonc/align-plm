...
class TranceptionBlockAttention(nn.Module):
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None, alibi_bias=None):
        ...
        if alibi_bias is not None:
            attn_weights = attn_weights + alibi_bias[:,:,:attn_weights.size(-1)].to(attn_weights.device)
...
class TranceptionLMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if self.retrieval_aggregation_mode is not None:
            ...
            self.MSA_log_prior = self.MSA_log_prior[self.config.context_s:self.config.context_e]
            self.MSA_start=max(config.MSA_start-config.context_s, 0)
            self.MSA_end=min(config.MSA_end-config.context_s,config.context_e-config.context_s)
            print(self.MSA_start,self.MSA_end,self.MSA_log_prior.size())
        else:
            pass
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.retrieval_aggregation_mode is not None:
                ...
                preds = torch.argmax(fused_shift_log_probas.view(-1, fused_shift_log_probas.size(-1)), dim=-1)
                labels = shift_labels.view(-1)
                accuracy = (preds == labels).float().sum() / (labels != -100).float().sum()
            else:
                loss_fct = CrossEntropyLoss()
                loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))

                preds = torch.argmax(shift_logits.view(-1, shift_logits.size(-1)), dim=-1)
                labels = shift_labels.view(-1)
                accuracy = (preds == labels).float().sum() / (labels != -100).float().sum()
    ...
    def score_mutants(self, DMS_data, target_seq=None, scoring_mirror=True, batch_size_inference=10, num_workers=10, indel_mode=False):
        ...
        df['mutated_sequence'] = df['mutated_sequence'].apply(lambda x: x[self.config.context_s:self.config.context_e])
        target_seq = target_seq[self.config.context_s:self.config.context_e]
        ...
        if target_seq in DMS_data.mutated_sequence.values:
            if scoring_mirror:
                wt_row = pd.DataFrame([[target_seq] + [0]*(len(all_scores.columns)-1)], columns=all_scores.columns)
            else:
                wt_row = pd.DataFrame([[target_seq] + [0]*(len(all_scores.columns)-1)], columns=all_scores.columns)
            all_scores = pd.concat([all_scores,wt_row], ignore_index=True)
        return all_scores

    def truncate_context(self, sequence):
        if isinstance(sequence, str):
            return sequence[self.config.context_s:self.config.context_e]
        elif isinstance(sequence, list):
            return [s[self.config.context_s:self.config.context_e] for s in sequence] 
        else:
            raise NotImplementedError
    
    def encode_batch_no_truncate(self, protein_sequence, sequence_name="sliced_mutated_sequence"):
        """
    ...
    def encode_batch(self, protein_sequence, sequence_name="sliced_mutated_sequence"):
        ...
        Method to process an input AA sequence batch (protein_sequence) and return a tokenized sequence (via the tokenizer associated to the model).
        """
        protein_sequence[sequence_name] = self.truncate_context(scoring_utils.sequence_replace(sequences=protein_sequence[sequence_name], char_to_replace='X', char_replacements='ACDEFGHIKLMNPQRSTVWY'))
        protein_sequence[sequence_name] = scoring_utils.sequence_replace(sequences=protein_sequence[sequence_name], char_to_replace='B', char_replacements='DN')
        protein_sequence[sequence_name] = scoring_utils.sequence_replace(sequences=protein_sequence[sequence_name], char_to_replace='J', char_replacements='IL')
        protein_sequence[sequence_name] = scoring_utils.sequence_replace(sequences=protein_sequence[sequence_name], char_to_replace='Z', char_replacements='EQ')
        return self.config.tokenizer(list(protein_sequence[sequence_name]), add_special_tokens=True, truncation=True, padding=True, max_length=self.config.n_ctx)

    def encode_batch_pair(self, data):
        p, d = self.truncate_context(data['preferred']), self.truncate_context(data['dispreferred'])

        p.extend(d)
        p = scoring_utils.sequence_replace(sequences=p, char_to_replace='X', char_replacements='ACDEFGHIKLMNPQRSTVWY')
        p = scoring_utils.sequence_replace(sequences=p, char_to_replace='B', char_replacements='DN')
        p = scoring_utils.sequence_replace(sequences=p, char_to_replace='J', char_replacements='IL')
        p = scoring_utils.sequence_replace(sequences=p, char_to_replace='Z', char_replacements='EQ')
        tokenized = self.config.tokenizer(p, add_special_tokens=True, truncation=True, padding=True, max_length=self.config.n_ctx)
        return {
            'preferred': tokenized[:len(p)//2],
            'dispreferred': tokenized[len(p)//2:]
        }

    def encode_batch_sft(self, data):
        p = self.truncate_context(data['msa_sequence'])
        p = scoring_utils.sequence_replace(sequences=p, char_to_replace='X', char_replacements='ACDEFGHIKLMNPQRSTVWY')
        p = scoring_utils.sequence_replace(sequences=p, char_to_replace='B', char_replacements='DN')
        p = scoring_utils.sequence_replace(sequences=p, char_to_replace='J', char_replacements='IL')
        p = scoring_utils.sequence_replace(sequences=p, char_to_replace='Z', char_replacements='EQ')
    ...
    def encode_batch_regression(self, data):
        p = self.truncate_context(data['mutated_sequence'])
        p = scoring_utils.sequence_replace(sequences=p, char_to_replace='X', char_replacements='ACDEFGHIKLMNPQRSTVWY')
        p = scoring_utils.sequence_replace(sequences=p, char_to_replace='B', char_replacements='DN')
        p = scoring_utils.sequence_replace(sequences=p, char_to_replace='J', char_replacements='IL')
        p = scoring_utils.sequence_replace(sequences=p, char_to_replace='Z', char_replacements='EQ')
