...
def get_tranception_scores_mutated_sequences(model, mutated_sequence_df, batch_size_inference, score_var_name, target_seq, num_workers=10, 
                                             reverse=False, indel_mode=False):
    """
    ...
    with torch.no_grad():
        ...
        ds.set_transform(model.encode_batch_no_truncate)
    ...
    if model.config.scoring_window=="sliding":
        scores = scores[['mutated_sequence','score']].groupby('mutated_sequence').sum().reset_index() 
